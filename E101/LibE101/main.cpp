// Library for ENGR101 robot challenge using Raspberry PI
// Author: A.Roberts, arthur.roberts@ecs.vuw.ac.nz
...
// how much diagnostic to display
int disp_level;
...
char get_pixel(int col, int row, int color)
{
    ...
    if (color==3)
    {
        char y = (pixels_buf[address] + pixels_buf[address+1] +pixels_buf[address+2])/3;
        return y;
    }
...
int set_pixel(int col, int row, char red, char green,char blue)
{
    int address = CAMERA_WIDTH*row*4 + col*4;
...
void GetLine(int row,int threshold)
{
	...
	printf("Running frame loop\n");
        ...
        cam->ReadFrame(texidx,&(pixels_buf[0]),sizeof(pixels_buf));


        int ind = 0;
		//int row = 25;
		int ini_row = CAMERA_WIDTH*row*4;
		char red,green,blue; //,a;

        printf("\n");
        int col = 0;
		for (ind = ini_row ; ind < ini_row + 4*CAMERA_WIDTH; ind = ind + 4)
		{
		   red =  pixels_buf[ind];
		   green =  pixels_buf[ind+1];
		   blue =  pixels_buf[ind+2];
		   //a = mybuff[ind+3];
           Y_row[col] = ((red+green+blue)/3) >threshold;
           col = col +1;
		  // printf("ind=%d red=%d green=%d blue=%d\n",ind,red,green,blue);
		}
		int j;
        ...
        {
            printf(" %d",Y_row[j]);
            line_error = line_error + ( j - CAMERA_WIDTH/2)*Y_row[j];
        }
        printf("\n");
        printf("line_error=%f\n",line_error);

        char v_cruise = 0;
        char v_left =  v_cruise + line_error/100.0;
        char v_right = v_cruise - line_error/100.0;

        printf(" v_left=%d v_right=%d\n",v_left,v_right);

        SetMotor(1,1,v_left);
        SetMotor(2,1,v_right);
